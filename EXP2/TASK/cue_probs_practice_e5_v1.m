...
%%%%%%%%%%%%% differs from exp 3 in that there is now one condition where
%%%%%%%%%%%%% the cue is easy to see (little blur) and one where it is
%%%%%%%%%%%%% harder (big blur)
%%%%%%%%%%%%% tgts on for 100 ms

% _______________________________________________________________________________________________________________
% DEPENDENCIES
% coded on Matlab 2013a using PTB v3.0.12 %%% CHECK THESE
% run on [INSERT MACHINE]
% with [INSERT MONITOR DETAILS]
...
if any(dbug)
    ...
    sess.blocks = [3 3 3 3]; % 1 = .4/.6 static, 2 = .4/.6 decay, 3 = .2/.8 static, 4 = .2/.8 decay
    sess.response_order = input('Response Order? ');
    sess.shapes = input('Shape Order? ');
end
sess.eye_on = 0;
...
% assign save mat
save_mat = ['KG_exp3_practice_cueprob_v5_1_' num2str(sess.sub_num) '_s' num2str(sess.session)];
...
task.trials_per_block = 20; %%%%% this is actually break (not block)
